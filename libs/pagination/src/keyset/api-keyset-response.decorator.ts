...
/**
 * Keyset 페이지네이션 목록 응답의 Swagger 스펙을 정의하는 데코레이터입니다.
 * `KeysetResponseDto`는 제네릭이라 CLI 플러그인이 `data` 필드 타입을 해석하지 못해
 ...
 * findAll(): Promise<KeysetResponseDto<PostDto>> { ... }
 */
export function ApiKeysetResponse<TModel extends Type<unknown>>(
  model: TModel,
  ...
  return applyDecorators(
    ApiExtraModels(KeysetResponseDto, model),
    ApiResponse({
      status,
      description,
      schema: {
