...
import {
  useResolveReport,
  useDismissReport,
} from '../hooks/use-admin-abuse-report-mutations';
import {
  ABUSE_REPORT_ACTION_LABELS,
  type AbuseReport,
...
const ACTIONS: AbuseReportAction[] = [
  'NO_ACTION',
  'WARN_USER',
  'HIDE_CONTENT',
  'DELETE_CONTENT',
...
export function AbuseReportResolveModal({ report, onClose }: Props) {
  const [actionTaken, setActionTaken] =
    useState<AbuseReportAction>('NO_ACTION');
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {mode === 'resolve' && (
          <div className="space-y-1">
            ...
            <select
              value={actionTaken}
              onChange={(e) => setActionTaken(e.target.value as AbuseReportAction)}
            ...
            >
              {ACTIONS.map((action) => (
                <option key={action} value={action}>
                  {ABUSE_REPORT_ACTION_LABELS[action]}
                </option>
              ))}
            </select>
          </div>
