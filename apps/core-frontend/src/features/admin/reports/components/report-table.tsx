...
const STATUS_VARIANT: Record<ReportStatus, 'default' | 'warning' | 'primary' | 'success' | 'error'> = {
  PENDING: 'warning',
  REVIEWING: 'primary',
  RESOLVED: 'success',
  DISMISSED: 'default',
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        >
          ...
          {(['POST', 'COMMENT', 'USER', 'MEDIA'] as ReportTarget[]).map((t) => (
            <option key={t} value={t}>{REPORT_TARGET_LABELS[t]}</option>
          ))}
        </select>
      </div>
                ...
                {data.map((report) => (
                  <tr key={report.id} className="transition-colors hover:bg-surface-2">
                    <td className="px-4 py-3">
                      <span className="text-text">{REPORT_TARGET_LABELS[report.targetType]}</span>
      ...
      <ReportResolveModal report={resolveReport} onClose={() => setResolveReport(null)} />
    </div>
  );
}
