...
jest.mock('../repositories/update-abuse-report.command', () => ({
  ResolveRelatedAbuseReportsCommand: jest.fn(),
}));
...
describe('ModerationService', () => {
  ...
  let prisma: any;
  const mockResolve = ResolveRelatedAbuseReportsCommand as jest.Mock;
  ...
  describe('deletePost / cascade 회귀 방지', () => {
    it('DeletePostCommand로 위임하여 자식까지 같은 tx에서 cascade', async () => {
      ...
      expect(prisma.$transaction).toHaveBeenCalledTimes(1);
      // tx 인자가 DeletePostCommand에도, ResolveRelatedAbuseReportsCommand에도 같은 prisma여야 한다
