...
## 🔭 OSS 공개를 위한 준비 (장기 북극성)

weaver2의 장기 방향은 **공개 OSS 보일러플레이트**입니다 ([`CHARTER.md §2`](CHARTER.md)). 현 단계 정체성은 "사내 팀용"이며, OSS로 가려면 아래 갭을 메워야 합니다. (지금 당장의 목표가 아니라, 갈 길을 적어두는 체크리스트입니다.)

1. **⚖️ 라이선스** — `LICENSE` 파일 추가 (MIT 등). 없으면 외부인이 합법적으로 사용 불가.
2. **📖 자기완결 문서** — 맥락 0에서 시작 가능한 온보딩(`docs/onboarding/` 진행 중) + `module-registry`를 README에 반영.
3. **🌐 범용화** — 최소 영어 README, 시드의 개인 디버깅 자산(leetspeak ID 등) 정리, 한국 특화(Kakao/Naver OAuth) 옵션화.
4. **🏗️ 간판 기능 완성** — `module-registry`를 banner POC에서 범용 동작으로 끌어올리기.
5. **🧪 성숙도 신호** — 테스트 커버리지 확대, CHANGELOG/semver 릴리스 태그, 데모/스크린샷.
6. **🤝 유지보수 약속** — 이슈/PR 응대 정책, 외부 기여 가이드 정비.
