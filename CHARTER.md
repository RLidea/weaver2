...
> 이 CRUD 패턴의 **레퍼런스 예시**일 뿐 본질이 아니다 — 커뮤니티·병원 EMR·SaaS는 도메인이 전혀 달라도
> 이 substrate 하나를 공유한다.

## 2. Target User

- **본인 + 사내 프로젝트** — 다양한 도메인(개인: 리데아 커뮤니티 · 회사: 병원 EMR·SaaS 등)의 공용 출발점.
- **Non-target** — 외부 OSS 일반 배포, 외주 협업사 단독 사용.

> 무게중심: 외부 일반 가독성보다 *6개월 뒤 본인·동료가 헷갈리지 않는 것*이 우선.
> 도메인마다 이 base를 **fork해서 얹는다** — 리데아·EMR·SaaS는 각자 이 base의 fork이고, 공용 base는 하나로 유지한다.
